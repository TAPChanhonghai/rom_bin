 ...
 *            @ref BSP_INIT_DK_SPI.
 *   Gxxx_DK: Use 0.
 *   STK:     Use @ref BSP_INIT_STK_BCUART to initialize board controller UART, 0
 *            otherwise.
 *
...
{
  uint16_t tmp;
  tmp  = BSP_RegisterRead(&BC_REGISTER->INTFLAG);
  tmp &= ~(flags);
  BSP_RegisterWrite(&BC_REGISTER->INTFLAG, tmp);
  return BSP_STATUS_OK;
}
...
static bool EbiInit(void)
{
  ...
  BSP_RegisterWrite(&BC_REGISTER->EBI_CTRL, 0x0001);
#endif
